...
export function useGeneratePresentation(notebookId?: string): Result {
  ...
  const pollGeneration = useCallback(
    async (
      jobId: string,
      initial: PresentationGenerationJobOut,
      runId: number,
    ): Promise<PresentationGenerationJobOut | null> => {
      ...
      while (Date.now() < deadline) {
        ...
        latest = await presentationsApi.getGenerationStatus(notebookId, jobId);
      }
  ...
  const generate = useCallback(
    async (payload: PresentationGenerateRequest) => {
      ...
      try {
        const job = await presentationsApi.generate(notebookId, payload);
        if (!isRunActive(runId)) {
          return null;
        }
        ...
        if (result.status === "failed") {
          if (isRunActive(runId)) {
            setError(result.error ?? "No se pudo generar la presentacion.");
          }
          return null;
        }
        return result;
  ...
  const resumeLatest = useCallback(
    async (options?: { suppressFailedError?: boolean }) => {
      ...
      try {
        const job = await presentationsApi.getLatestGeneration(notebookId);
        if (!isRunActive(runId)) {
          return null;
        }
        ...
        if (job.status === "failed") {
          if (!suppressFailedError && isRunActive(runId)) {
            setError(job.error ?? "No se pudo generar la presentacion.");
          }
          return null;
        }
